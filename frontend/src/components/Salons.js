...
function Salons() {
  ...
  const [noSalonsMessage, setNoSalonsMessage] = useState("");
    ...
    const fetchSalons = async () => {
      try {
        ...
        if (response.data.message) {
          setNoSalonsMessage(response.data.message);
          setSalons([]);
        } else {
  ...
  const filteredSalons = salons.filter((salon) => {
    ...
    const salonNameMatch = salon.salonName
      ? salon.salonName.toLowerCase().includes(searchTerm.toLowerCase())
      : false;

    const ownerMatch = salon.owner
      ? salon.owner.toString().toLowerCase().includes(searchTerm.toLowerCase())
      : false;

    return salonNameMatch || ownerMatch;
  });
    ...
    return (
      <div className="container mx-auto p-6">
        <div className="text-center bg-yellow-50 rounded-lg p-8">
          <div className="text-yellow-800 text-xl font-semibold mb-2">
            No Salons Found
          </div>
          <p className="text-yellow-600">{noSalonsMessage}</p>
        </div>
      </div>
    );
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="text-2xl font-bold text-green-600">
            {salons.reduce(
              (acc, salon) => acc + (salon.services?.length || 0),
              0
            )}
          </div>
          ...
          <div className="text-2xl font-bold text-purple-600">
            {
              salons.filter(
                (salon) =>
                  salon.openingHours?.sunday?.open !== "closed" &&
                  salon.openingHours?.sunday?.open !== undefined
              ).length
            }
          </div>
            ...
            >
              <div className="p-6">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <p className="text-gray-600 flex items-center">
                      ...
                      Owner ID: {salon.owner}
                    </p>
                  </div>
                  <span className="px-3 py-1 bg-green-100 text-green-800 rounded-full text-sm">
                    {salon.isDeleted ? "Inactive" : "Active"}
                  </span>
                </div>
                ...
                <div className="space-y-2 text-gray-600">
                  ...
                  <p className="text-gray-600">{salon.bio}</p>
                </div>
              ...
              {salon.openingHours && (
                <div className="bg-gray-50 px-6 py-4">
                  ...
                  <p className="text-gray-600">
                    {salon.openingHours[
                      Object.keys(salon.openingHours)[new Date().getDay()]
                    ]?.open || "Closed"}{" "}
                    -
                    {salon.openingHours[
                      Object.keys(salon.openingHours)[new Date().getDay()]
      ...
      {filteredSalons.length > 0 && (
        <div className="mt-8 flex justify-center">
          ...
          >
            ...
            {[...Array(Math.ceil(filteredSalons.length / salonsPerPage))].map(
              (_, index) => (
                <button
                  key={index + 1}
            ...
            <div className="flex flex-col">
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  ...
                  {Object.entries(selectedSalon.workingHours).map(
                    ([day, hours]) => (
